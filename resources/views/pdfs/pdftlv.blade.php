...
<body>
    ...
    <div style="font-size: 15px;margin-top: 5px;">
        <p style="font-size: 20px;">Proprietaire : {{$data[0]['contrat']['appartement']['proprietaire']['id'] ?? '--'}} ---- {{$data[0]['contrat']['appartement']['proprietaire']['prenom'] ?? '----' }} {{$data[0]['contrat']['appartement']['proprietaire']['nom'] ?? '----' }} </p>

        @for ($i = 0; $i < count($data); $i++)
        <p style="font-size: 16px;">  Locataire : 

        @if($data[$i]['contrat']['locataire']['nomentreprise'])
            <span>{{ $data[$i]['contrat']['locataire']['nomentreprise']}} </span>
        @else
            <span>{{ $data[$i]['contrat']['locataire']['prenom']}}</span>
            <span>{{ $data[$i]['contrat']['locataire']['nom']}}</span>
        ...
        <table style="font-size: 15px;" class="table">
            <thead style="border-bottom: 1px solid black;">
                <tr>
                    ...
                    <td style="font-size: 15px;">Logement</td>
                    <td style="font-size: 15px;">Loyer de base</td>
                    <td style="font-size: 15px;">TLV</td>
                </tr>
            </thead>
            <tbody>
                    <tr style="font-size: 15px">
                        <td style="font-size: 15px;">{{$data[$i]['contrat']['appartement']['nom'] ?? '---' }}</td>
                        <td style="font-size: 15px;">{{$data[$i]['montantloyerbase_avenant'] ?? 0}}</td>
                        <td style="font-size: 15px;">{{$data[$i]['contrat']['appartement']['tlvmontant'] ?? '----' }}</td>
                    </tr>
            </tbody>
            <br>
        </table>
