...
<head>
    ...
    <title>APPEL LOYER {{date('d/m/Y')}}</title>
...
<body>
    ...
    <div style="font-size: 13px;font-weight: bold;text-align: right;margin: 30px 0 10px">
        ADRESSE :
        @if ($data[0]['locataire'] && $data[0]['locataire']['nomentreprise'])
        {{$data[0]['locataire']['adresseentreprise']}}
        @else
        {{($data[0]['appartement'] && $data[0]['appartement']['immeuble']) ? $data[0]['appartement']['immeuble']['adresse'] : "Adresse"}}
        @endif
   ...
   <p>
    ...
    <div style="font-size: 15px;text-align: left;margin: 30px 0 5px;">
        ...
        <p>- {{count($data[0]['periodes_non_payes']) > 1 ? "Loyers" : "Loyer"}} @foreach ( $data[0]['periodes_non_payes'] as $item)
            {{$item['designation']}},
            @endforeach
            {{date_format(now() , "Y")}}
        </p>
            ...
            <tbody>
                <tr>
                    <td colspan="2" style="font-size:15px;">
                        Soit <b>{{$data[0]['total_loyer_format']}} F x {{count($data[0]['periodes_non_payes'])}} </b>
                    </td>
                    <td colspan="" style="font-size:15px;font-weight:bold"> = </td>
                    <td colspan="" style="font-size:15px;font-weight:bold">
                        {{ number_format(($data[0]['total_loyer'] * count($data[0]['periodes_non_payes']) ), 0, '', ' ') }} F
