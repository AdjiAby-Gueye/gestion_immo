@if(auth()->user()->can('liste-paiementecheance') || auth()->user()->can('modification-paiementecheance') || auth()->user()->can('suppression-paiementecheance'))
        ...
        <div class="col-span-12 mt-6">
            <div class="intro-y block sm:flex items-center h-10">
                <h2 class="text-lg font-medium truncate mr-5 uppercase">
                    Paiement Echeance
                    <span class="inline-block bg-white text-theme-106 ml-2 px-3 rounded-full">@{{paginations['paiementecheance'].totalItems}}</span>
                </h2>

                <div class="flex items-center sm:ml-auto mt-3 sm:mt-0">
                    @if(auth()->user()->can('creation-paiementecheance'))
                            ...
                            <div class="dropdown relative">
                                ...
                                <div class="dropdown-box mt-40 absolute top-0 right-0 z-20" style="width: 125px">
                                    <div class="dropdown-box__content box p-2">
                                        <button class="button  flex items-center text-gray-700" ng-click="showModalAdd('paiementecheance')" title="Ajouter"> Un Element</button>
                                        ...
                                        <div class="accordion__pane__content mt-2 text-gray-700 leading-relaxed">
                                            <form class="bg-white grid p-3 mt-3">
                                                <div class="flex flex-wrap -mx-3">
                                                    <div class="w-full md:w-1/2 px-3 mb-2">
                                                        <div class="inline-block relative w-full">
                                                            <select class="block appearance-none w-full bg-white text-gray-700 border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline"
                                                                    id="searchoption_list_paiementecheance" name="searchoption">
                                                                <option value="">Rechercher par</option>
                                                                <option selected value="montant">montant</option>
                                                    ...
                                                    <div class="w-full md:w-1/2 px-3 mb-2">
                                                        <input class="shadow appearance-none border w-full  rounded py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="searchtexte_list_paiementecheance" ng-model="searchtexte_list_paiementecheance" autocomplete="off" type="text" placeholder="Texte ... ">
                                                    </div>
                                                    <div class="w-full md:w-1/2 px-3 mb-2">
                                                        <div class="inline-block relative w-full">
                                                            <select class="select2 block appearance-none w-full bg-white text-gray-700 border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline"
                                                                    id="locataire_list_paiementecheance">
                                                    ...
                                                    <div class="w-full md:w-1/1 px-3 md:mb-0 mt-2">
                                                        <div class="flex flex-wrap -mx-3">
                                                            <div class="w-full md:w-1/2 px-3 md:mb-0">
                                                                ...
                                                                <a ng-if="filters" href="generate-excel-paiementecheance/@{{filters}}" target="_blank"   class="button bg-success text-white btn-shadow" title="Excel"><span class="fas fa-file-excel mr-1"></span>EXCEL</a>
                                                                <a ng-if="!filters" href="generate-excel-paiementecheance" target="_blank"   class="button bg-success text-white btn-shadow" title="Excel"><span class="fas fa-file-excel mr-1"></span>EXCEL</a>
                                                            </div>
                                                            <div class="w-full md:w-1/2 px-3 md:mb-0 text-right">
                                                                <button type="button" class="button small border text-gray-700 mr-1 btn-shadow-dark" ng-click="emptyform('paiementecheance', true)"><span class="fa fa-filter mr-1"></span>Annuler</button></button>
                                                                <button type="button" class="button bg-theme-101 text-white btn-shadow" ng-click="pageChanged('paiementecheance')"><span class="fa fa-search mr-1"></span>Filtrer</button>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
            ...
            <div class="overflow-table">
                <div class="intro-y overflow-auto lg:overflow-visible mt-8 sm:mt-0">
                    <table class="table table-report sm:mt-2">
                        ...
                        <tbody>
                            <tr class="intro-x" ng-repeat="item in dataPage['paiementecheances']" >
                                <td>
                                    <div class="font-medium whitespace-no-wrap">@{{ item.avisecheance.contrat.locataire.prenom }} @{{ item.avisecheance.contrat.locataire.nom }}/lot: @{{ item.avisecheance.contrat.appartement.lot}}/ilot: @{{ item.avisecheance.contrat.appartement.ilot.numero}}</div>
                                </td>
                                <td>
                                    <div class="font-medium whitespace-no-wrap">@{{ item.date }}</div>
                                </td>
                                <td>
                                    <div class="font-medium whitespace-no-wrap">@{{ item.periodes }}</div>
                                </td>
                                ...
                                <td>
                                    <div class="font-medium whitespace-no-wrap text-center">@{{ item.modepaiement.designation }}  </div>
                                </td>
                                ...
                                <td class="table-report__action w-56">
                                    <nav class="menu-leftToRight uk-flex text-center">
                                        ...
                                        <label class="menu-open-button bg-white" for="menu-open1us-@{{ item.id }}">
                                            ...
                                            <button class="menu-item btn border-0 bg-info text-white fsize-16"  ng-click="showModalUpdate('paiementecheance',item.id)" title="Modifier les infos">
                                                <span class="fal fa-edit"></span>
                                            </button>
                                        
                                           
                                            <button class="menu-item btn border-0 bg-danger text-white fsize-16" ng-click="deleteElement('paiementecheance',item.id)"  title="Supprimer">
            ...
            <div class="col-span-12 grid grid-cols-12 gap-4 mt-3">
                <div class="col-span-12 sm:col-span-12 md:col-span-3">
                    ...
                    <select class="w-20 input box mt-1" ng-model="paginations['paiementecheance'].entryLimit" ng-change="pageChanged('paiementecheance')">
                ...
                <div class="col-span-12 sm:col-span-12 md:col-span-9" style="float:right">
                    <nav aria-label="Page navigation">
                        <ul class="uk-pagination float-right" uib-pagination total-items="paginations['paiementecheance'].totalItems" ng-model="paginations['paiementecheance'].currentPage" max-size="paginations['paiementecheance'].maxSize" items-per-page="paginations['paiementecheance'].entryLimit" ng-change="pageChanged('paiementecheance')" previous-text="‹" next-text="›" first-text="«" last-text="»" boundary-link-numbers="true" rotate="false"></ul>
                    </nav>
                </div>
            </div>
            <!-- /PAGINATION -->
