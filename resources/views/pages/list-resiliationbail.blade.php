@if(auth()->user()->can('liste-resiliationbail') || auth()->user()->can('modification-resiliationbail') || auth()->user()->can('suppression-resiliationbail'))
        ...
        <div class="col-span-12 mt-6">
            ...
            <div class="sm:flex  items-center">
                ...
                <div class="intro-y block sm:flex  flex-1 items-center theme-dark border-raduis-right">
                    <h2 class="text-lg font-medium truncate mr-5 uppercase ml-2">
                        @{{titlePage}}
                        <span class="inline-block bg-white text-theme-106 ml-2 px-3 rounded-full">@{{paginations['resiliationbail'].totalItems}}</span>
                    </h2>
                    <div class="flex items-center sm:ml-auto mt-3 sm:mt-0">
                        @if(auth()->user()->can('creation-resiliationbail'))
                                ...
                                <div class="dropdown relative">
                                    ...
                                    <div class="dropdown-box mt-40 absolute left-1/2 transform  top-20 z-50 " style="width: 140px">
                                        <div class="dropdown-box__content box  p-2">
                                            <button class="button dropdown-box-btn   flex items-center text-gray-700" ng-click="showModalAdd('resiliationbail')" title="Ajouter"> Un Element</button>
                                            <button class="button dropdown-box-btn flex items-center text-gray-700" ng-click="showModalAdd('resiliationbail', {is_file_excel:true, title: 'resiliationbail'})" title="Import"> Fichier Excel</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                                    ...
                                                    <div class="w-full md:w-1/1 px-3 md:mb-0 mt-2">
                                                        <div class="flex flex-wrap -mx-3">
                                                            <div class="w-full md:w-1/2 px-3 md:mb-0">
                                                                <a ng-if="filters" href="generate-pdf-user/@{{filters}}" target="_blank" class="button small border text-white bg-danger mr-1 btn-shadow-dark" title="PDF"><span class="fas fa-file-pdf mr-1"></span>PDF</a>
                                                            ...
                                                            <div class="w-full md:w-1/2 px-3 md:mb-0 text-right">
                                                                <button type="button" class="button small border text-gray-700 mr-1 btn-shadow-dark" ng-click="emptyform('resiliationbail', true)"><span class="fa fa-filter mr-1"></span>Annuler</button></button>
                                                                <button type="button" class="button bg-theme-101 text-white btn-shadow" ng-click="pageChanged('resiliationbail')"><span class="fa fa-search mr-1"></span>Filtrer</button>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                    ...
                    <table class="table table-report sm:mt-2">
                        <thead>
                            <tr>
                                ...
                                <th class="whitespace-no-wrap text-center">Nom locataire</th>
                                <th class="whitespace-no-wrap text-center">Nom de l'immeuble</th>
                                <th class="whitespace-no-wrap text-center">Nom de l'appartement</th>
                                <th class="whitespace-no-wrap text-center">Date de la demande</th>
                                <th class="text-center whitespace-no-wrap">Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr class="intro-x" ng-repeat="item in dataPage['resiliationbails']">
                                <td class="w-40">
                                ...
                                <td class="table-report__action w-56">
                                    <nav class="menu-leftToRight uk-flex text-center">
                                        ...
                                        <button class="menu-item btn border-0 bg-danger text-white fsize-16" ng-click="deleteElement('resiliationbail',item.id)" title="Supprimer">
                                            <span class="fa fa-trash-alt"></span>
                                        </button>
                                        <button class="menu-item btn border-0 bg-info text-white fsize-16" ng-click="showModalUpdate('resiliationbail',item.id)" title="Modifier les infos">
            ...
            <div class="col-span-12 grid grid-cols-12 gap-4 mt-3">
                <div class="col-span-12 sm:col-span-12 md:col-span-3">
                    ...
                    <select class="w-20 input box mt-1" ng-model="paginations['resiliationbail'].entryLimit" ng-change="pageChanged('resiliationbail')">
                ...
                <div class="col-span-12 sm:col-span-12 md:col-span-9" style="float:right">
                    <nav aria-label="Page navigation">
                        <ul class="uk-pagination float-right" uib-pagination total-items="paginations['resiliationbail'].totalItems" ng-model="paginations['resiliationbail'].currentPage" max-size="paginations['resiliationbail'].maxSize" items-per-page="paginations['resiliationbail'].entryLimit" ng-change="pageChanged('resiliationbail')" previous-text="‹" next-text="›" first-text="«" last-text="»" boundary-link-numbers="true" rotate="false"></ul>
                    </nav>
                </div>
            </div>
            <!-- /PAGINATION -->
