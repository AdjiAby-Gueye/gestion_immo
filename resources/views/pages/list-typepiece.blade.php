@if(auth()->user()->can('liste-typepiece') || auth()->user()->can('modification-typepiece') || auth()->user()->can('suppression-typepiece'))
            ...
            <div class="col-span-12 mt-6">
                <div class="sm:flex items-center">
                    <div class="p-3 intro-y theme-sombre rounded-2xl">
                        <i class="fa tooltip image_fit fa-copy" aria-hidden="true"></i>
                    </div>
                    <div class="intro-y block sm:flex flex-1 items-center theme-dark border-raduis-right">
                        <h2 class="text-lg font-medium truncate mr-5 uppercase ml-2">
                            Types de pièces
                            <span class="inline-block bg-white text-theme-106 ml-2 px-3 rounded-full">@{{paginations['typepiece'].totalItems}}</span>
                        </h2>
                        <div class="flex items-center sm:ml-auto mt-3 sm:mt-0">
                            @if(auth()->user()->can('creation-typepiece'))
                                    ...
                                    <div class="dropdown relative">
                                        ...
                                        <div class="dropdown-box mt-40 absolute left-1/2 transform top-20 z-50" style="width: 140px">
                                            <div class="dropdown-box__content box p-2">
                                                <button class="button dropdown-box-btn flex items-center text-gray-700" ng-click="showModalAdd('typepiece')" title="Ajouter">
                                                    Un Élément
                                                </button>
                                                <button class="button dropdown-box-btn flex items-center text-gray-700" ng-click="showModalAdd('typepiece', {is_file_excel:true, title: 'type de pièce'})" title="Import">
                                                    Fichier Excel
                                                </button>
                                            </div>
                                        </div>
                                                    ...
                                                    <div class="flex flex-wrap -mx-3">
                                                        <div class="w-full md:w-1/2 px-3 mb-36">
                                                            <div class="inline-block relative w-full">
                                                                <select class="select2 block appearance-none w-full bg-white text-gray-700 border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline" 
                                                                        id="searchoption_list_typepiece" ng-model="searchoption_list_typepiece" name="searchoption">
                                                        ...
                                                        <div class="w-full md:w-1/2 px-3 mb-2">
                                                            <input class="shadow appearance-none border w-full rounded py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" 
                                                                   id="searchtexte_list_typepiece" ng-model="searchtexte_list_typepiece" autocomplete="off" type="text" placeholder="Texte ... ">
                                                        </div>

                                                        <div class="w-full md:w-1/1 px-3 md:mb-0 mt-3">
                                                            <div class="flex flex-wrap -mx-3">
                                                                <div class="w-full md:w-1/2 px-3 md:mb-0">
                                                                    <a ng-if="filters" href="generate-pdf-typepiece/@{{filters}}" target="_blank" class="custom-btn rounded-full small bg-danger mr-1 btn-shadow-dark" title="PDF">
                                                                        <span class="fas fa-file-pdf mr-1"></span>PDF
                                                                    </a>
                                                                    <a ng-if="!filters" href="generate-pdf-typepiece" target="_blank" class="custom-btn rounded-full small bg-danger mr-1 btn-shadow-dark" title="PDF">
                                                                        <span class="fas fa-file-pdf mr-1"></span>PDF
                                                                    </a>
                                                                    <a ng-if="filters" href="generate-excel-typepiece/@{{filters}}" target="_blank" class="custom-btn rounded-full btn-shadow" title="Excel">
                                                                        <span class="fas fa-file-excel mr-2"></span>EXCEL
                                                                    </a>
                                                                    <a ng-if="!filters" href="generate-excel-typepiece" target="_blank" class="custom-btn rounded-full btn-shadow" title="Excel">
                                                                        <span class="fas text-success fa-file-excel mr-2"></span>EXCEL
                                                                    </a>
                                                                </div>

                                                                <div class="w-full md:w-1/2 px-3 md:mb-0 text-right">
                                                                    <button type="button" class="custom-btn small border mr-1 btn-shadow-dark" ng-click="emptyform('typepiece', true)">
                                                                        <span class="fa icon-i fa-filter mr-1"></span>Annuler
                                                                    </button>
                                                                    <button type="button" class="custom-btn bg-theme-101 btn-shadow" ng-click="pageChanged('typepiece')">
                ...
                <div class="overflow-table">
                    <div class="intro-y overflow-auto lg:overflow-visible mt-8 sm:mt-0">
                        <table class="table table-report sm:mt-2">
                            ...
                            <tbody>
                                <tr class="intro-x" ng-repeat="item in dataPage['typepieces']">
                                    <td>
                                    ...
                                    <td class="table-report__action w-56">
                                        <nav class="menu-leftToRight uk-flex text-center">
                                            ...
                                            <button class="menu-item btn border-0 bg-danger text-white fsize-16" ng-click="deleteElement('typepiece',item.id)" title="Supprimer">
                                                <span class="fa fa-trash-alt"></span>
                                            </button>
                                            <button class="menu-item btn border-0 bg-info text-white fsize-16" ng-click="showModalUpdate('typepiece',item.id)" title="Modifier les infos">
                ...
                <div class="col-span-12 grid grid-cols-12 gap-4 mt-3">
                    <div class="col-span-12 sm:col-span-12 md:col-span-3">
                        ...
                        <select class="w-20 input box mt-1" ng-model="paginations['typepiece'].entryLimit" ng-change="pageChanged('typepiece')">
                    ...
                    <div class="col-span-12 sm:col-span-12 md:col-span-9" style="float:right">
                        <nav aria-label="Page navigation">
                            <ul class="uk-pagination float-right" uib-pagination total-items="paginations['typepiece'].totalItems" ng-model="paginations['typepiece'].currentPage" max-size="paginations['typepiece'].maxSize" items-per-page="paginations['typepiece'].entryLimit" ng-change="pageChanged('typepiece')" previous-text="‹" next-text="›" first-text="«" last-text="»" boundary-link-numbers="true" rotate="false"></ul>
                        </nav>
                    </div>
                </div>
                <!-- /PAGINATION -->
