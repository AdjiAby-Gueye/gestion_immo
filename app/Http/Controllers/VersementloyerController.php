...
class VersementloyerController extends SaveModelController
{
    //extends BaseController

    use AuthorizesRequests, DispatchesJobs, ValidatesRequests;

    protected $queryName = "versementloyers";
    protected $model = Versementloyer::class;
    ...
    public function save(Request $request)
    {
        try {
            return DB::transaction(function () use ($request) {
              ...
              //  dd($request) ;
                $item = new Versementloyer();

                if (isset($request->id)) {
                    if (is_numeric($request->id) == true) {
                        $item = Versementloyer::find($request->id);
                ...
                if (empty($request->contrat)) {
                    $errors = "l'appartement n'a pas de contrat actuellement";
                }
                if (empty($request->proprietaire)) {
                    $errors = "l'appartement n'a pas de proprietaire actuellement";
                }
                ...
                if (empty($request->debut)) {
                    $errors = "Veuillez renseigner le debut de la periode de validité";
                }
                if (empty($request->fin)) {
                    $errors = "Veuillez renseigner la fin de la periode de validité";
                }
                ...
                if($request->file('document')) {
                    ...
                    $path = $request->file('document')->storeAs('uploads/versementloyers', $fileNameToStore);
                    $fichier = isset($_FILES['document']['name']) ? $_FILES['document']['name'] : "";
                    if (!empty($fichier)) {
                        $dateHeure = date('Y_m_d_H_i_s');
                        $fichier_tmp = $_FILES['document']['tmp_name'];
                        $ext = explode('.', $fichier);
                        $rename = config('view.uploads')[$this->queryName] . "/documentversementloyer_" . $dateHeure . "." . end($ext);
              ...
              //  $contrat = Contrat::find($request->contrat);
