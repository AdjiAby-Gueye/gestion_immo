...
class AppartementApiController extends Controller
{
    public function index()
    {
        $appartements = Appartement::with(['immeuble','typeappartement','locataire','etatappartement'])->get() ;
        return response($appartements, 200) ;
    }
   ...
   /* public function store(Request $request)
    ...
    {
        $appartement = Appartement::with(['immeuble','typeappartement','locataire','etatappartement'])->find($id);
        if (is_null($appartement))
        {
            return response()->json(['error' => true, 'message' => "Appartement inéxistant", ], 404);
        }
