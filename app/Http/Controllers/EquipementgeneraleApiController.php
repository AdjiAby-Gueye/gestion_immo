...
class EquipementgeneraleApiController extends Controller
{
    public function index()
    {
        $compositions = Detailcomposition::with(['immeuble','typeappartement','locataire','etatappartement'])->get() ;
   ...
   /* public function store(Request $request)
    ...
    {
        $equipementgenerale = Detailcomposition::with(['equipement','appartement'])->where('appartement_id',$id)->get();
        if (is_null($equipementgenerale))
        {
            return response()->json(['error' => true, 'message' => "equipement inexistants", ], 404);
        }
        return response()->json($equipementgenerale, 200);
    }
