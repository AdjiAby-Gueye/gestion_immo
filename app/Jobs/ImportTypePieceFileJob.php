...
class ImportTypePieceFileJob implements ShouldQueue
{
    ...
    public function handle()
    {
      ...
      //      dd($filename) ;
            ...
            DB::transaction(function () use (&$totalUpload, &$data, &$report, &$lastItem)
            {
                for ($i=1;$i < count($data);$i++)
                {
                    ...
                    try
                    {
                        ...
                        $get_type             = isset($row[1]) ? trim($row[1]) : null;
                   ...
                   // dd($report) ;
                    ...
                    $get_piece                            = Typepiece::whereRaw('TRIM(lower(designation)) = TRIM(lower(?))',["$get_designation"])->first();
                   ...
                   // dd($errors);
                    if(!$errors)
                    {
                        if(!isset($get_piece))
                        {
                            $get_piece    = new Typepiece();
                        }

                        $get_piece->designation = $get_designation;
                        $get_piece->iscommun = 1;
          ...
          //  dd($lastItem) ;
            Outil::atEndUploadData($this->pathFile, $this->generateLink, $report, $this->user, $totalToUpload, $totalUpload, " des types pieces appartement", $lastItem);
        }
        catch (\Exception $e)
        {
            try
