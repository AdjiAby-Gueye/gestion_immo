...
class ImportTypecontratFileJob implements ShouldQueue
{
    ...
    public function handle()
    {
      ...
      //      dd($filename) ;
            ...
            DB::transaction(function () use (&$totalUpload, &$data, &$report, &$lastItem)
            {
                for ($i=1;$i < count($data);$i++)
                {
                   ...
                   // dd($report) ;
                    ...
                    $newtypecontratTypecontrat                    = Typecontrat::whereRaw('TRIM(lower(designation)) = TRIM(lower(?))',["$get_designation"])->first();

                   // dd($errors);
                    if(!$errors)
                    {
                        if(!isset($newtypecontratTypecontrat))
                        {
                            $newtypecontratTypecontrat    = new Typecontrat();
                        }

                        $newtypecontratTypecontrat->designation = $get_designation;
                        

                        $is_save = $newtypecontratTypecontrat->save();

                        $lastItem = $newtypecontratTypecontrat;
                    }
                    if($is_save)
                    {
                        $totalUpload ++;
          ...
          //  dd($lastItem) ;
            Outil::atEndUploadData($this->pathFile, $this->generateLink, $report, $this->user, $totalToUpload, $totalUpload, " des types contratTypecontrat", $lastItem);
        }
        catch (\Exception $e)
        {
            try
